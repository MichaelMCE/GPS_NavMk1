...
#define ILI9341_PINK 0xF81F /*0xFE19 */
...
protected:
	...
	void drawFastVLine_noTrans (const int16_t x, const int16_t y, const uint16_t h, const uint16_t color);
...
public:
	...
	void drawFastVLine(const int16_t x, const int16_t y, const uint16_t h, const uint16_t color);
	...
	__attribute__((always_inline)) void setScroll (uint16_t offset)
	{
		beginTransaction();
		enableCS();
	...
	__attribute__((always_inline)) void drawFill_start (const uint16_t w, const uint16_t h, const uint16_t colour)
	{
		const uint32_t totalPixels = (uint32_t)w * (uint32_t)h;
		fillScanline16(colour, min(totalPixels, SCANLINE_PIXEL_COUNT));
		//fillScanline16(colour, min(SCANLINE_PIXEL_COUNT, max(w, h)));
		beginTransaction();
		enableCS();
	}

	__attribute__((always_inline)) void drawPixel_start ()
	{
		beginTransaction();
		enableCS();
	}

	__attribute__((always_inline)) void draw_end ()
	{
		disableCS();
		endTransaction();
	}

	__attribute__((always_inline)) void drawPixel_write (const int x, const int y, const uint16_t colour)
	{
		setAddrAndRW_cont_inline(x, y);
		setDCForData();
		write16_cont(colour);
	}

	__attribute__((always_inline)) void drawPixel_write4 (const uint16_t x, const uint16_t y, const uint16_t colour)
	{
		setAddrAndRW_cont_inline4(x, y);
		setDCForData();
		write16_cont(colour);
		write16_cont(colour);
		write16_cont(colour);
	...
	__attribute__((always_inline)) void drawPixel_write9 (const uint16_t x, const uint16_t y, const uint16_t colour)
	{
		setAddrAndRW_cont_inline9(x, y);
		setDCForData();
		write16_cont(colour);
		write16_cont(colour);
		write16_cont(colour);
	...
	__attribute__((always_inline)) void fillCircleStart_start (const uint16_t w, const uint16_t h, const uint16_t colour)
	{
		const uint32_t totalPixels = (uint32_t)w * (uint32_t)h;
		fillScanline16(colour, min(totalPixels, SCANLINE_PIXEL_COUNT));
		beginTransaction();
	...
	__attribute__((always_inline)) void drawFastVLine_noTrans_noFill (const int16_t x, const int16_t y, const uint16_t h)
	{
	...
	__attribute__((always_inline)) void drawFastVLine_noTrans_noCS (const int16_t x, const int16_t y, const uint16_t h, const uint16_t color)
	{
		// Rudimentary clipping
		//if ((x >= _width) || (y >= _height)) return;
		//if ((y + (int16_t)h - 1) >= _height) h = _height - y;

		fillScanline16(color, h/*min(h, SCANLINE_PIXEL_COUNT)*/);
	...
	__attribute__((always_inline)) void fillCircleHelper_noCS (int16_t x0, int16_t y0, int16_t r, uint8_t cornername, int16_t delta, uint16_t color)
	{
		int16_t f = 1 - r;
		int16_t ddF_x = 1;
		int16_t ddF_y = -2 * r;
	...
	__attribute__((always_inline)) void fillCircle_writeA (int16_t x0, int16_t y0, int16_t r, uint16_t color)
	{
		drawFastVLine_noTrans_noFill(x0, y0 - r, 2 * r + 1);
		fillCircleHelper_noCS(x0, y0, r, 3, 0, color);
	}

	__attribute__((always_inline)) void fillCircle_writeB (int16_t x0, int16_t y0, int16_t r, uint16_t color)
	{
		drawFastVLine_noTrans_noCS((int16_t)(x0), (int16_t)(y0 - r), (uint16_t)(2 * (int16_t)r + 1), color);
		//drawVLine_write(x, y, h);
		fillCircleHelper_noCS(x0, y0, r, 3, 0, color);
	}
		
	__attribute__((always_inline)) void drawRect_write (int16_t x, int16_t y, uint16_t w, uint16_t h)
	{
		drawHLine_write(x, y, w);
		drawVLine_write(x, y, h);
		drawVLine_write(x + w - 1, y, h);
		drawHLine_write(x, y + h - 1, w);
	}


	__attribute__((always_inline)) void drawRectFilled_write (const int16_t x, const int16_t y, const uint16_t w, const uint16_t h)
	{
	...
	__attribute__((always_inline)) void drawVLine_write (const int16_t x, const int16_t y, const uint16_t h)
	{
		setAddrAndRW_cont(x, y, 1, h);
		setDCForData();
		writeScanline16(h);
	...
	__attribute__((always_inline)) void drawHLine_write (const int16_t x, const int16_t y, const uint16_t w)
	{
	...
	void setSPIClockDivider(const uint8_t divider);
	...
	inline gTextFont getFont()
	{
		return _font;
	}
	
	inline void setColour (uint16_t color)
	{
		_fontColor = color;
	}
	...
	inline uint16_t getTextBackgroundColor()
	{
		return _fontBgColor;
	}
	...
	inline uint8_t getTextLetterSpacing()
	{
	...
	inline uint8_t getTextLineSpacing()
	{
		return _lineSpacing;
	}
	...
	inline gTextFontMode getFontMode()
	{
		return _fontMode;
	}
	...
	__attribute__((always_inline)) uint8_t getFontHeight()
	{
		return pgm_read_byte(_font + GTEXT_FONT_HEIGHT);
	}

	__attribute__((always_inline)) static uint8_t getFontHeight(gTextFont font)
	{
		return pgm_read_byte(font + GTEXT_FONT_HEIGHT);
	}

	inline uint8_t readcommand8(uint8_t c)
	{
	...
	inline static uint16_t color565(uint8_t r, uint8_t g, uint8_t b)
	{
	...
	inline void setCursor (const int16_t x, const int16_t y)
	{
		cursorToXY(x, y);
	}
	
	inline void getCursor (int16_t *x, int16_t *y)
	{
		*x = _x;
		*y = _y;
	}
	...
	static uint16_t getCharWidth (uint8_t c, const gTextFont font, const uint8_t textScale)
	{
		int16_t width = 0;
	...
	static uint16_t getStringWidth (const char* str, const gTextFont font, const uint8_t letterSpacing, const uint8_t textScale)
	{
		uint16_t width = 0;
	...
	__attribute__((always_inline)) void fillArc (const uint16_t x, const uint16_t y, const uint16_t radius, const uint16_t thickness, const float start, const float end, const uint16_t color)
	{
		beginTransaction();
		if (start == 0 && end == _arcAngleMax)
			fillArcOffsetted(x, y, radius, thickness, 0, _arcAngleMax, color);
		else
			fillArcOffsetted(x, y, radius, thickness, start + (_angleOffset / 360.0f)*_arcAngleMax, end + (_angleOffset / 360.0f)*_arcAngleMax, color);
		endTransaction();
	}
	...
	inline float cosDegrees (const float angle)
	{
	...
	inline float sinDegrees (const float angle)
	{
		//float radians = angle / (float)360 * 2 * PI;
		//Serial << "SIN_LOOKUP angle:" << (float)angle << " radians:" << radians << " sin:" << sin(radians) << " return: " << sin(radians) * (double)65535 << endl;
	...
	__attribute__((always_inline)) void beginTransaction ()
	{
#ifdef ILI_USE_SPI_TRANSACTION
...
protected:
	...
	__attribute__((always_inline)) uint16_t scaledFontHeight ()
	{
		return (uint16_t)(pgm_read_byte(_font + GTEXT_FONT_HEIGHT)) * (uint16_t)_textScale;
	}


	__attribute__((always_inline)) void spiwrite (const uint8_t c)
	{
#if defined ARDUINO_ARCH_AVR
		SPDR = c;
		asm volatile("nop");
	...
	__attribute__((always_inline)) void spiwrite16 (const uint16_t d)
	{
#if defined ARDUINO_ARCH_AVR
		SPDR = highByte(d);
		while (!(SPSR & _BV(SPIF)));
	...
	__attribute__((always_inline)) void write8_cont (const uint8_t c)
	{
#if SPI_MODE_NORMAL
		spiwrite(c);
#elif SPI_MODE_EXTENDED
	...
	// CS and DC have to be set prior to calling this method
	__attribute__((always_inline)) void write8_last (const uint8_t c)
	{
#if SPI_MODE_NORMAL
		spiwrite(c);
		disableCS();
	...
	__attribute__((always_inline)) void write16_cont (const uint16_t d)
	{
#if SPI_MODE_NORMAL
		spiwrite16(d);
#elif SPI_MODE_EXTENDED
	...
	__attribute__((always_inline)) void write16_last (const uint16_t d)
	{
#if SPI_MODE_NORMAL
		spiwrite16(d);
		disableCS();
	...
	__attribute__((always_inline)) void setAddr_cont (const uint16_t x, const uint16_t y, const uint16_t w, const uint16_t h)
	{
	...
	__attribute__((always_inline)) void setAddrAndRW_cont (const uint16_t x, const uint16_t y, const uint16_t w, const uint16_t h)
	{
		setDCForCommand();
		write8_cont(ILI9341_CASET); // Column addr set
		setDCForData();
	...
	__attribute__((always_inline)) void setAddrAndRW_cont_inline (const uint16_t x, const uint16_t y, const uint16_t w, const uint16_t h)
	{
		setDCForCommand();
		write8_cont(ILI9341_CASET); // Column addr set
		setDCForData();
	...
	__attribute__((always_inline)) void setAddrAndRW_cont_inline4 (const uint16_t x, const uint16_t y)
	{
		setDCForCommand();
		write8_cont(ILI9341_CASET);		// Column addr set
		setDCForData();
		write16_cont(x);				// XSTART
		write16_cont(x+1);				// XEND
		setDCForCommand();
		write8_cont(ILI9341_PASET);		// Row addr set
		setDCForData();
		write16_cont(y);				// YSTART
		write16_cont(y+1);				// YEND
		setDCForCommand();
		write8_cont(ILI9341_RAMWR);		// RAM write
	}
		
	__attribute__((always_inline)) void setAddrAndRW_cont_inline9 (const uint16_t x, const uint16_t y)
	{
		setDCForCommand();
		write8_cont(ILI9341_CASET);		// Column addr set
		setDCForData();
		write16_cont(x);				// XSTART
		write16_cont(x+2);				// XEND
		setDCForCommand();
		write8_cont(ILI9341_PASET);		// Row addr set
		setDCForData();
		write16_cont(y);				// YSTART
		write16_cont(y+2);				// YEND
		setDCForCommand();
		write8_cont(ILI9341_RAMWR);		// RAM write
	}
	
	__attribute__((always_inline)) void setAddrAndRW_cont_inline (const int x, const int y)
	{
		setDCForCommand();
		write8_cont(ILI9341_CASET); // Column addr set
		setDCForData();
	...
	__attribute__((always_inline)) void setColumnAddr (const uint16_t x, const uint16_t w)
	{
		setDCForCommand();
		write8_cont(ILI9341_CASET); // Column addr set
		setDCForData();
	...
	__attribute__((always_inline)) void setRowAddr (const uint16_t y, const uint16_t h)
	{
		setDCForCommand();
		write8_cont(ILI9341_PASET); // Row addr set
		setDCForData();
	...
	__attribute__((always_inline)) void setRW ()		// RAM Write
	{
		setDCForCommand();
		write8_cont(ILI9341_RAMWR); // RAM write
	}
	...
	// Enables CS, sets DC for Command, writes 1 byte
	// Does not disable CS
	__attribute__((always_inline)) void writecommand_cont (const uint8_t c)
	{
		setDCForCommand();
#if SPI_MODE_NORMAL | SPI_MODE_DMA
		enableCS();
	...
	__attribute__((always_inline)) void writecommand_last (const uint8_t c)
	{
		setDCForCommand();
#if SPI_MODE_NORMAL | SPI_MODE_DMA
		enableCS();
	...
	// Does not disable CS
	__attribute__((always_inline)) void writedata8_cont (const uint8_t c)
	{
		setDCForData();
#if SPI_MODE_NORMAL | SPI_MODE_DMA
		enableCS();
	...
	__attribute__((always_inline)) void writedata8_last (const uint8_t c)
	{
		setDCForData();
#if SPI_MODE_NORMAL | SPI_MODE_DMA
		enableCS();
	...
	__attribute__((always_inline)) void write_cont (uint16_t *buf, const uint32_t n)
	{
#if SPI_MODE_NORMAL
		spiTransfer(buf, n);
#elif SPI_MODE_EXTENDED
	...
	__attribute__((always_inline)) void write_cont (const uint16_t *buf, uint32_t n)
	{
#if SPI_MODE_NORMAL
		spiTransfer(buf, n);
#elif SPI_MODE_EXTENDED
	...
	__attribute__((always_inline)) void read_cont (uint8_t *buf, const uint32_t n)
	{
#if SPI_MODE_DMA
		dmaReceive(buf, n);
#endif
...
//	}
	...
	__attribute__((always_inline)) void writeScanline16 (const uint32_t n)
	{
		/*setDCForData();
		enableCS();*/
#if SPI_MODE_NORMAL
	...
	__attribute__((always_inline)) void writeScanlineLooped (const uint32_t n)
	{
		int32_t numLoops = n / (int32_t)SCANLINE_PIXEL_COUNT;
		while(numLoops--)
	...
	// Reads 1 byte
	__attribute__((always_inline)) uint8_t read8_cont ()
	{
#if SPI_MODE_NORMAL
	...
	__attribute__((always_inline)) uint8_t read8_last ()
	{
#if SPI_MODE_NORMAL
	...
	// Reads 2 bytes
	__attribute__((always_inline)) uint16_t read16 ()
	{
#if SPI_MODE_NORMAL
	...
	// Reads 2 bytes
	__attribute__((always_inline)) uint16_t readPixel_start_cont ()
	{
#if SPI_MODE_NORMAL
		uint16_t r = SPI.transfer(ILI9341_NOP);
		r <<= 8;
	...
	// Does not disable CS
	__attribute__((always_inline)) uint8_t readdata8_cont ()
	{
		setDCForData();
#if SPI_MODE_NORMAL | SPI_MODE_DMA
		enableCS();
	...
	__attribute__((always_inline)) uint8_t readdata8_last ()
	{
		setDCForData();
#if SPI_MODE_NORMAL | SPI_MODE_DMA
		enableCS();
	...
	// Does not disable CS
	__attribute__((always_inline)) uint16_t readdata16_cont ()
	{
		setDCForData();
#if SPI_MODE_NORMAL | SPI_MODE_DMA
		enableCS();
	...
	__attribute__((always_inline)) uint8_t readdata16_last ()
	{
		setDCForData();
#if SPI_MODE_NORMAL | SPI_MODE_DMA
		enableCS();
	...
	__attribute__((always_inline)) void fillScanline16 (const uint16_t color)
	{
		for (uint16_t i = 0; i < SCANLINE_PIXEL_COUNT; i++)
			_scanline16[i] = color;
	}

	__attribute__((always_inline)) void fillScanline16 (const uint16_t color, const uint16_t len)
	{
...
//#endif
	...
	__attribute__((always_inline)) void writeHLine_cont_noCS_noFill (const int16_t x, const int16_t y, const int16_t w)
	{
#ifdef ARDUINO_ARCH_AVR
	...
	__attribute__((always_inline)) void writeHLine_cont_noCS_noScanline (const int16_t x, const int16_t y, int16_t w, const uint16_t color)
	{
		setAddrAndRW_cont(x, y, w, 1);
		setDCForData();
		while (w-- > 0) write16_cont(color);
	}
	...
	// the scanlineBuffer not filled on every call
	__attribute__((always_inline)) void writeVLine_cont_noCS_noFill (const int16_t x, const int16_t y, const int16_t h)
	{
#ifdef ARDUINO_ARCH_AVR
	...
	__attribute__((always_inline)) void writeVLine_cont_noCS_noScanline (const int16_t x, const int16_t y, int16_t h, const uint16_t color)
	{
		setAddrAndRW_cont(x, y, 1, h);
		setDCForData();
		while (h-- > 0) write16_cont(color);
	}

	__attribute__((always_inline)) void writePixel_cont (const int16_t x, const int16_t y, const uint16_t color)
	{
	...
	__attribute__((always_inline)) void writePixel_last (const int16_t x, const int16_t y, const uint16_t color)
	{
	...
	__attribute__((always_inline)) void drawPixel_cont (const int16_t x, const int16_t y, const uint16_t color)
	{
	...
	__attribute__((always_inline)) void drawPixel_last (const int16_t x, const int16_t y, const uint16_t color)
	{
		if ((x < 0) || (x >= _width) || (y < 0) || (y >= _height))
			return;
...
public:
	// Enables CS
	__attribute__((always_inline)) void enableCS()
	{
#if SPI_MODE_NORMAL | SPI_MODE_DMA
		*_csport &= ~_cspinmask;
#endif
	...
	// Disables CS
	__attribute__((always_inline)) void disableCS ()
	{
#if SPI_MODE_NORMAL | SPI_MODE_DMA
		*_csport |= _cspinmask;
...
protected:
	...
	// Sets DC to Data (1)
	__attribute__((always_inline)) void setDCForData ()
	{
		*_dcport |= _dcpinmask;
	...
	__attribute__((always_inline)) void setDCForCommand ()
	{
		*_dcport &= ~_dcpinmask;
	}
	
#ifdef ARDUINO_ARCH_AVR
	__attribute__((always_inline)) void spiTransfer (const uint8_t *buf, uint32_t count)
	{
		SPDR = *buf;
		while (--count > 0) {
			uint8_t out = *(buf + 1);
	...
	__attribute__((always_inline)) void spiTransfer (uint16_t *buf, uint32_t count)
	{
		uint16_t pixel = *buf;
		SPDR = highByte(pixel);
		while (!(SPSR & _BV(SPIF)));
	...
	inline void spiTransfer (const uint16_t *buf, uint32_t count)
	{
	...
	inline void spiTransfer (const uint8_t _data)
	{
	...
	inline void spiTransfer (const uint16_t _data)
	{
		uint32_t ch = BOARD_PIN_TO_SPI_CHANNEL(BOARD_SPI_DEFAULT_SS);
	...
	inline void spiTransfer (const uint8_t *_buf, uint32_t _count)
	{
		if (_count == 0) return;
		if (_count == 1) {
			spiTransfer(*_buf);
	...
	inline void spiTransfer (const uint16_t *_buf, uint32_t _count)
	{
		if (_count == 0) return;
		if (_count == 1){
			spiTransfer(*_buf);
	...
	inline void spiTransfer (const byte _pin, const uint8_t *_buf, uint32_t _count, const SPITransferMode _mode)
	{
		if (_count == 0) return;
		if (_count == 1){
	...
	inline void spiTransfer (const byte _pin, uint16_t _data, const SPITransferMode _mode)
	{
		uint32_t ch = BOARD_PIN_TO_SPI_CHANNEL(_pin);
	...
	inline void spiTransfer (const byte _pin, const uint16_t *_buf, uint32_t _count, const SPITransferMode _mode)
	{
		if (_count == 0) return;
		if (_count == 1) {
...
#define ILI_SAM3X_DMA_TIMEOUT 20
...
#define ILI_SPI_RX_IDX  2
	...
	/** Disable DMA Channel. */
	inline static void dmac_channel_disable (const uint32_t ul_num)
	{
		DMAC->DMAC_CHDR = DMAC_CHDR_DIS0 << ul_num;
	}
	
	/** Enable DMA Channel. */
	inline static void dmac_channel_enable (const uint32_t ul_num)
	{
		DMAC->DMAC_CHER = DMAC_CHER_ENA0 << ul_num;
	}
	
	/** Poll for transfer complete. */
	inline static bool dmac_channel_transfer_done (const uint32_t ul_num)
	{
		return (DMAC->DMAC_CHSR & (DMAC_CHSR_ENA0 << ul_num)) ? false : true;
	}

	inline static void spi_set_16bit_transfer ()
	{
		SPI0->SPI_CSR[ILI_SPI_CHIP_SEL] = (SPI0->SPI_CSR[ILI_SPI_CHIP_SEL] &= 0xFFFFFF0F) | 0x00000080;
	}

	inline static void spi_set_8bit_transfer ()
	{
	...
	void dmaInit (const uint8_t sckDivisor)
	{
	...
	void dmaInit16 (const uint8_t sckDivisor)
	{
		uint8_t scbr = sckDivisor;
		Spi* pSpi = SPI0;
		//  disable SPI
	...
	//------------------------------------------------------------------------------
	// start RX DMA
	void spiDmaRX (uint8_t *dst, const uint16_t count)
	{
	...
	//------------------------------------------------------------------------------
	// start TX DMA
	void spiDmaTX (const uint8_t *src, const uint16_t count)
	{
	...
	void spiDmaTX16 (const uint16_t* src, const uint16_t count)
	{
	...
	//------------------------------------------------------------------------------
	__attribute__((always_inline)) uint8_t dmaSpiTransfer (uint8_t b)
	{
		Spi* pSpi = SPI0;
	...
	__attribute__((always_inline)) uint16_t dmaSpiTransfer (uint16_t w)
	{
	...
	//------------------------------------------------------------------------------
	/** SPI receive a byte */
	__attribute__((always_inline)) uint8_t dmaReceive ()
	{
		return dmaSpiTransfer((uint8_t)0XFF);
	}
	//------------------------------------------------------------------------------
	/** SPI receive multiple bytes */
	uint8_t dmaReceive (uint8_t *buf, const uint32_t n)
	{
		Spi* pSpi = SPI0;
		int rtn = 0;
#if ILI_USE_SAM3X_DMAC
	...
	//------------------------------------------------------------------------------
	/** SPI send a byte */
	__attribute__((always_inline)) void dmaSend (const uint8_t b)
	{
		dmaSpiTransfer(b);
	}

	__attribute__((always_inline)) void dmaSend (const uint16_t w)
	{
		dmaSpiTransfer(w);
	}
	
	//------------------------------------------------------------------------------
	void dmaSend (const uint8_t* buf, const uint32_t n)
	{
		Spi* pSpi = SPI0;
		spiDmaTX(buf, n);
	...
	void dmaSend (const uint16_t* buf, const uint32_t n)
	{
		Spi* pSpi = SPI0;
